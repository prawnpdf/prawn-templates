...
describe 'Document built from a template' do
  ...
  it "does not set the template page's parent to the document pages catalog (especially with nested pages)" do
  ...
  it 'does not add an extra restore_graphics_state operator to the end of any content stream' do
    filename = "#{DATADIR}/pdfs/curves.pdf"
  ...
  it 'has a single page object if importing a single page template' do
    ...
    pages = hash.values.select { |obj| obj.is_a?(Hash) && obj[:Type] == :Page }
  ...
  it 'allows PDFs with page resources behind an indirect object to be used as templates' do
...
describe 'ObjectStore extensions' do
  ...
  it 'imports all objects from a PDF that has an indirect reference in a stream dict' do
    filename = "#{DATADIR}/pdfs/indirect_reference.pdf"
    store = PDF::Core::ObjectStore.new(template: filename)
    expect(store.size).to eq 8
  end

  it 'raises error ArgumentError when given a file that doesn exist as a template' do
    filename = 'not_really_there.pdf'

    expect { PDF::Core::ObjectStore.new(template: filename) }.to raise_error(ArgumentError)
  end

  it 'raises error PDF::Core::Errors::TemplateError when given a non PDF as a template' do
  ...
  it 'raises error PDF::Core::Errors::TemplateError when given an encrypted PDF as a template' do
...
describe 'ObjectStore#object_id_for_page' do
  ...
  it 'returns the object ID of the first page of a template that uses nested Pages' do
    filename = "#{DATADIR}/pdfs/nested_pages.pdf"
    store = PDF::Core::ObjectStore.new(template: filename)
    expect(store.object_id_for_page(1)).to eq 5
  end

  it 'returns the object ID of the last page of a template that uses nested Pages' do
    filename = "#{DATADIR}/pdfs/nested_pages.pdf"
    store = PDF::Core::ObjectStore.new(template: filename)
    expect(store.object_id_for_page(-1)).to eq 8
  end

  it 'returns nil if given an invalid page number' do
    filename = "#{DATADIR}/pdfs/hexagon.pdf"
    store = PDF::Core::ObjectStore.new(template: filename)
